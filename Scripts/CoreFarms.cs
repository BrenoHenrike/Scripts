...
public class CoreFarms
{
    ...
    public void BerserkerBunny(int goldQuant = 100000000)
    {
		...
		while (Bot.Player.Gold < goldQuant || Bot.Player.Gold <= 100000000)
		{
            ...
            Core.HuntMonster("greenguardwest", "Big Bad Boar", "Were Egg");
	...
	public void IcestormArena(int level = 100, bool rankUpClass = false)
	{
        ...
        while (Bot.Player.Level < 5 && Bot.Player.Level < level)
			Core.KillMonster("icestormarena", "r4", "Left", "*");

        while (Bot.Player.Level < 10 && Bot.Player.Level < level)
			Core.KillMonster("icestormarena", "r5", "Left", "*");
		
		while (Bot.Player.Level < 20 && Bot.Player.Level < level)
            Core.KillMonster("icestormarena", "r6", "Left", "*");

		while (Bot.Player.Level < 25 && Bot.Player.Level < level)
		{
            Core.EnsureAccept(6628);
            Core.KillMonster("icestormarena", "r7", "Left", "*", "Icewing Grunt Defeated", 3);
            Core.EnsureComplete(6628);
		}

        while (Bot.Player.Level < 30 && Bot.Player.Level < level)
            Core.KillMonster("icestormarena", "r10", "Left", "*");

        while (Bot.Player.Level < 35 && Bot.Player.Level < level)
        {
            Core.EnsureAccept(6629);
            Core.KillMonster("icestormarena", "r11", "Left", "*", "Icewing Warrior Defeated", 3);
            Core.EnsureComplete(6629);
        }

		while(Bot.Player.Level < 50 && Bot.Player.Level < level)
            Core.KillMonster("icestormarena", "r14", "Left", "*");

		while(Bot.Player.Level < 75 && Bot.Player.Level < level)
            Core.KillMonster("icestormarena", "r3b", "Top", "*");

		while(Bot.Player.Level < 100 && Bot.Player.Level < level)
            Core.KillMonster("icestormarena", "r3c", "Top", "*");
    }
