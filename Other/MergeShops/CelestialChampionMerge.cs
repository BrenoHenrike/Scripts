...
public class CelestialChampionMerge
{
    ...
    {
        Core.BankingBlackList.AddRange(new[] { "Lumin Badge", "Champion Sash " });
        Core.SetOptions();
        ...
        void findIngredients()
        {
            ...
            switch (req.Name)
            {
                ...
                case "Champion Sash":
                    ...
                    while (!Bot.ShouldExit && !Core.CheckInventory(req.Name, quant))
                    {
                        Core.HuntMonster("celestialarenad", "Aranx", req.Name, quant, isTemp: false);
                        Bot.Wait.ForPickup(req.Name);
                    }
                    break;
