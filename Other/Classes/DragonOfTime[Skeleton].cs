...
public class DragonOfTime
{
    ...
    public void GetDoT(bool rankUpClass = true, bool doExtra = true)
    {
        ...
        Core.Logger("You thought you were sneaky huh, downloading this unreleased skeletal bot. Well, it's not a master yet, so you will need to do these things yourself:", messageBox: true);
        Bot.Sleep(1000);
        Core.Logger("Get the Pink Blade of Destruction", messageBox: true);
        Bot.Sleep(1000);
        Core.Logger("Do the story of /GuardianTree", messageBox: true);
        Bot.Sleep(1000);
        Core.Logger("Do the story of /Whitehole", messageBox: true);
        Bot.Sleep(1000);
        Core.Logger("Do the story of /Collection", messageBox: true);
        Bot.Sleep(1000);
        Core.Logger("Get 100x Darkon's Receipt", messageBox: true);
        Bot.Sleep(1000);
        Core.Logger("Do the story of /Borgars", messageBox: true);
        Bot.Sleep(1000);
        Core.Logger("If you dont, expect the bot to break ☺. Also don't expect it to be bugfree", messageBox: true);
        ...
        if (!Core.QuestProgression(7718))
        {
            ...
            ///PBoD.GetPBoD(); Pink Blade of Destruction
        ...
        if (!Core.QuestProgression(7721))
        {
            ...
            if (!Core.CheckInventory("Starlight Singularity"))
            {
                ...
                Core.EquipClass(ClassType.Solo);
                Core.HuntMonster("whitehole", "Mehen", "Mehen Slain");
