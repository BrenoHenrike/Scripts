...
public class project
{
    ...
    public void Enhance(bool bank = false)
    {
        ...
        // List<string> BankItems = Bot.Bank.Items.FindAll(x => x.EnhancementLevel < Bot.Player.Level && (x.ItemGroup != "None" && x.ItemGroup != "am" && x.ItemGroup != "co" && x.ItemGroup != "pe")).Select(x => x.Name).ToList();
        ...
        if (bank)
        {
            foreach (string item in InventoryItems)
            {
                ...
                // string[] BankItems = Bot.Bank.Items.FindAll(x => BankItems.Contains(x.Name) && x.Coins).Select(x => x.Name).ToArray();
