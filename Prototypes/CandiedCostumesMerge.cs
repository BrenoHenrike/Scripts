...
public class CandiedCostumesMerge
{
    ...
    {
        Core.BankingBlackList.AddRange(new[] { "MarshMeowllows", "Horehound Bits", "Kitty Cordials", "Candied Jalapeno", "Spicy Sample "});
        Core.SetOptions();
        ...
        void findIngredients()
        {
            ...
            switch (req.Name)
            {
                ...
                case "Horehound Bits":
                    ...
                    while (!Bot.ShouldExit && !Core.CheckInventory(req.Name, quant))
                    {
                        Core.HuntMonster("chromafection", "Baby", "Gummy Babies", 10);
