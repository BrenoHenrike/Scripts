...
public class RVAArmy
{
    ...
    {
        if (item != null && isTemp ? Bot.Inventory.ContainsTempItem(item, quant) : Core.CheckInventory(item, quant))
            return;
        if (!isTemp && item != null)
            Core.AddDrop(item);
...
// public void AFKCheck(ScriptInterface bot)
// {
//     Bot.Sleep(2000);
//     Bot.Events.PlayerAFK += (ScriptInterface bot) => Bot.SendPacket("%xt%zm%afk%1%false%");
// }
